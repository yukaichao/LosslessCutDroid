...
> v0.6.5：重写 XML 界面，新增三种选区方式、慢放、逐帧模式、导出参数页和编码导出能力探测。
...
- 新增自定义时间轴拖动框。
- 新增逐帧模式：进入全屏后用系统解码器缓存当前位置附近的原始分辨率帧，支持左右半屏点按切帧和缩放拖动确认画面。
...
- 新增“预览输入 / 预览输出”按钮。
- 裁剪成功后自动切换到输出文件预览。
- 导出改为通过文件描述符直读输入、直写输出，并在写入输出时优先使用可截断的 `rwt` 模式，避免新裁剪文件比旧文件短时残留旧文件尾部。
